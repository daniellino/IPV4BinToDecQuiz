...
var resetWholeGeneration = function resetWholeGeneration() {
...
for (var i = 0; i < 8; i++) {
  ...
  binar.push(result); // console.log(binar);

  _final = _final.concat(result);
}

var binaryVlaue = document.getElementById("binary-Value");
binaryVlaue.innerHTML = _final; //convert to decial value

for (var _i = 0; _i < binar.length; _i++) {
  if (binar[_i] == 1) {
    decimVal = decimVal + Math.pow(2, 7 - _i); // console.log(Math.pow(2, 7 - i));
...
var clickedObjectContainer;
var myChosenAnswers = ''; //variable for keeping concatenated words

var givenAns = document.getElementById('combAns'); //~~~~~~~~~~~~~~~~~asked question loop~~~~~~~~~~~~~~~~
...
function testForHidingRow() {
  var tablRowCount = document.querySelector('.answersTable').rows.length;
  console.log(tablRowCount);

  for (var Indx = 1; Indx < tablRowCount + 1; Indx++) {
    if (document.querySelector('.answersTable tr:nth-child(' + Indx + ') td:nth-child(2)>a').style.display == 'none' && document.querySelector('.answersTable tr:nth-child(' + Indx + ') td:nth-child(3)>a').style.display == 'none') {
      document.querySelector('.answersTable tr:nth-child(' + Indx + ')').style.display = 'none';
    } //end of if
...
var myArray = [decimVal]; //*********defining array of answers
...
var newArray = shuffleArray(myArray); //*********************checking the shuffled array for empty elements
...
function runToCheck() {
  ...
  var resultAns = document.getElementById('matchField');
  ...
  function myFunctionFalse() {
    ...
    if (myChosenAnswers == '') {
      givenAns.innerHTML = "Correct answer is: ".concat(decimVal, ", and you didn't choose an option");
    } else {
      givenAns.innerHTML = "Correct answer is: ".concat(decimVal, ", You Chose ").concat(myChosenAnswers, "!");
    }
  } //end of function myFunctionFalse
  //~~~~~~~~~~~~~~~~Main condition~~~~~~~~~~~~~////////////////////////////////


  var cmbAnsFieldContent = givenAns.innerText;
...
for (var q = 0; q < newArray.length; q++) {
  var indexStr = String(q + 1); // console.log(newArray.length);

  indexStr = 'print_arr' + indexStr; // console.log(indexStr);

  document.getElementById(indexStr).innerHTML = String(newArray.filter(testForempty)[q]);
}